...
export const FormLabel = defineComponent({
  ...
  setup(props, { slots }) {
    const field = inject<FormFieldContextValue | null>(FORM_FIELD_CONTEXT_KEY, null)
    const className = computed(() => cn('text-sm font-medium leading-none', field?.error && 'text-red-500', props.class))
    return () => h('label', { for: props.for, class: className.value }, slots.default?.())
  },
})
...
export const FormDescription = defineComponent({
  ...
  props: { class: { type: String, default: '' } },
  setup(props, { slots }) {
    return () => h('p', { class: cn('text-sm text-gray-500', props.class) }, slots.default?.())
  },
})
...
export const FormMessage = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () =>
      body.value
        ? h('p', { class: cn('text-sm font-medium text-red-500', props.class) }, body.value || slots.default?.())
        : null
  },
})
