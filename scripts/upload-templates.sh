...
# 在远程服务器上解压文件并重启 rcoder 容器
echo "正在远程解压文件..."

sshpass -p "$REMOTE_PASSWORD" ssh -o StrictHostKeyChecking=no \
    "$REMOTE_USER@$REMOTE_HOST" << EOF
cd $REMOTE_PATH
echo "解压 react-vite-template.zip..."
echo "$REMOTE_PASSWORD" | sudo -S rm -rf react-vite-template
sudo unzip -o -q react-vite-template.zip -d react-vite-template
echo "✓ react-vite-template 解压完成"

echo "解压 vue3-vite-template.zip..."
sudo rm -rf vue3-vite-template
sudo unzip -o -q vue3-vite-template.zip -d vue3-vite-template
echo "✓ vue3-vite-template 解压完成"

echo ""
echo "模板目录内容:"
ls -lh react-vite-template/ vue3-vite-template/ 2>/dev/null | head -20
